...
void NaoBehavior::beam(double &beamX, double &beamY, double &beamAngle)
{
	...
	case WO_TEAMMATE3:
		...
		break;
	case WO_TEAMMATE4:
		beamX = -HALF_FIELD_X / 1.2;
		beamY = 0;
		break;
	//前锋*7
	case WO_TEAMMATE5:
		beamX = -HALF_FIELD_X / 1.6;
...
SkillType NaoBehavior::selectSkill()
{
	...
	*/
	...
	case 4:
		passPosition = VecPosition(worldModel->getUNum(), 0, 0);
		cout << "I am 4，I heard:" << endl;
		for (int i = 1; i <= 3; i++)
		{
			cout << i << " will kick ball to " << worldModel->passBalls[i - 1] << endl;
		}
		cout << "But 2 or 3 has no ball, so I will go to " << worldModel->passBalls[0] << "to kick ball." << endl;
	default:
		break;
	}
